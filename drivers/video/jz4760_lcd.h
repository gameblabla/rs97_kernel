 ...
 */
/* AUO */
#if defined(CONFIG_JZ4760_LCD_AUO_A030JTN01V2)
#if defined(CONFIG_JZ4760_LEPUS) || defined(CONFIG_JZ4760B_LEPUS)/* board pavo */
	#define SPEN		(32*1+29)       /*LCD_CS*/
	#define SPCK		(32*1+28)       /*LCD_SCL*/
	#define SPDA		(32*1+21)       /*LCD_SDA*/
...
#define __lcd_special_on()			     \
	do {					     \
		...
		__gpio_set_pin(LCD_RET);	     \
		/* rs97 v2.0 */ \
		__spi_write_reg(0x05, 0x34); \
		udelay(200); \
		__spi_write_reg(0x05, 0x74); \
		__spi_write_reg(0x95, 0x00); \
		__spi_write_reg(0x1b, 0xb9); \
		__spi_write_reg(0x1c, 0x4b); \
		__spi_write_reg(0x9a, 0x00); \
		__spi_write_reg(0x05, 0x75); \
		printk("init rs97 screen (AUO A030JTN01.2) complete\n"); \
} while (0)
...
#endif	/* JZ4760_LCD_AUO_A030JTN01V2 */
